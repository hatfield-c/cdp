...
struct Rigidbody {
	...
	void AirResistance(Vector3 wind) {
		...
		Vector3 velocity_drag_force = this->velocity * - 0.5f * (1.293e-3f) * 0.47f * Math::Pi() * (0.25f * 0.25f);
		Vector3 wind_drag_force = wind * -0.5f * (1.293e-3f) * 0.47f * Math::Pi() * (0.25f * 0.25f);
