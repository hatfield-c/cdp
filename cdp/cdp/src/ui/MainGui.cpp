...
MainGui::MainGui() {
    glfwSetErrorCallback(MainGui::glfw_error_callback);
    if (!glfwInit()) {
    ...
    init_info.CheckVkResultFn = MainGui::check_vk_result;
...
void MainGui::Loop() {
    ...
    while (!glfwWindowShouldClose(this->window))
    {
        ...
        if (!is_minimized)
        {
            ...
            this->FrameRender(this->wd, draw_data);
            this->FramePresent(this->wd);
        }
    }
...
void MainGui::glfw_error_callback(int error, const char* description)
{
    fprintf(stderr, "GLFW Error %d: %s\n", error, description);
}
void MainGui::check_vk_result(VkResult err)
{
    if (err == 0)
        return;
...
void MainGui::SetupVulkan(ImVector<const char*> instance_extensions)
{
    VkResult err;
...
void MainGui::SetupVulkanWindow(ImGui_ImplVulkanH_Window* wd, VkSurfaceKHR surface, int width, int height)
{
    wd->Surface = surface;
...
void MainGui::CleanupVulkanWindow()
{
    ImGui_ImplVulkanH_DestroyWindow(g_Instance, g_Device, &g_MainWindowData, g_Allocator);
}

void MainGui::FrameRender(ImGui_ImplVulkanH_Window* wd, ImDrawData* draw_data)
{
    VkResult err;
...
void MainGui::FramePresent(ImGui_ImplVulkanH_Window* wd)
{
